...
export default function AdminOrders() {
  ...
  const updateOrder = async (productId) => {
    ...
    // console.log("hello")
    try {
      var res = await updateOrderByAdmin(model);
      //   console.log(res);
      if (res.status == 201) {
        // setOrders(res.data.orders);
        Notification("Order Department", res.data.message, "Error");
  ...
  //   let output =
  // console.log(orders[0].orderStatus.reverse().find(status => {
          ...
          >
            <div class="TableRow-sc-1sslxri-0 fFiRPC">
              ...
              <div
                ...
                cursor="unset"
              >
                {orderEdit != "" && orderEdit.orderStatus[1].isCompleted && (
                  <select
                    className="FormInput"
                    onChange={(e) => setType(e.target.value)}
                  >
                    <option value={""}>select status</option>
                    {orderEdit.orderStatus.map((status) => {
                      return (
                        <>
                          {!status.isCompleted ? (
